...
# Caddy's own systemd drop-in -- and of the two download pages. Nothing else under /etc is copied: another
...
tar -czf "$DEST/.partial.$$" -C / etc/caddy/Caddyfile etc/systemd/system/caddy.service.d root/sequentia/downloads/index.html root/sequentia/downloads/core/index.html
