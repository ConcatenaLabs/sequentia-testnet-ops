...
# Caddy's own systemd drop-in -- and of the download page. Nothing else under /etc is copied: another
...
tar -czf "$DEST/.partial.$$" -C / etc/caddy/Caddyfile etc/systemd/system/caddy.service.d root/sequentia/downloads/index.html
